...
\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
    \item Thesis (tentative): ``\textit{AutoTex: Reducing Uncoalesced Global Memory Traffic on GPUs Using Texture Memory}''
...
\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
    \item Developed a defect detection pipeline using \textbf{YOLOv5}, contributing to automated yield analysis.
    \item Built data dashboards with \textbf{Apache Superset} to visualize real-time factory production and downtime metrics.
\end{itemize}

\begin{center}
    \textbf{\Large TEACHING ASSISTANTSHIPS}\raggedright \,\hrulefill
\end{center}
...
\begin{itemize}[noitemsep, topsep=0pt, partopsep=0pt, parsep=0pt, leftmargin=*]
    \item Developed and deployed a \textbf{RESTful API} to ensure robust backend communication.
    \item Integrated \textbf{SQLAlchemy ORM} with MariaDB for efficient data handling.
    \item Achieved \textbf{99\% test coverage} using unit tests within an automated \textbf{CI workflow}.
\end{itemize}
